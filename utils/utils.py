...
def get_gradient(graph):
    ...
    x, edge_index, _, _ = decompose_graph(graph)
    
    return x[edge_index[1], ...] - x[edge_index[0], ...]
    
def edge_dot_product(vector1, vector2, edge_index, num_nodes):
    """
    ...
    """
    dot_ = vector1 * vector2
    return scatter_add(dot_, edge_index[0], dim=0, dim_size=num_nodes)
...
def csr_to_tensor(csr_matrix):
    ...
    indices = torch.LongTensor([row, col])    # torch.int64
    values = torch.FloatTensor(csr_matrix.data)    # torch.float32
    
    return torch.sparse.FloatTensor(indices, values, torch.Size(csr_matrix.shape))
