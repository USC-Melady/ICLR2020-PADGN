...
#         self.reset_parameters()
    ...
    def forward(self, graph):
        ...
        if self._update_graph:
            ...
            _graph = copy_geometric_data(graph)
            
        if self._use_edge_block:
            # Edge-level update
            _graph = self.edge_model_block(_graph)    # (N^e, d_e+d_v+d_v+d_g) -> (N^e, out_features)
        
        if self._use_node_block:
            # Node-level update
            _graph = self.node_model_block(_graph)    # (N^v, d_v+d_e+d_e+d_g) -> (N^v, out_features)
        
        if self._use_global_block:
            # Global-level update
            _graph = self.global_model_block(_graph)
